...
class Folk_RNN:
    ...
    def compose_tune(self, nobars):
        """
        Composes tune and returns it as abc notation
        """
        tune = list(self.tune)
        baridxs = self.findbaridxs()
        while tune[-1] != self.end_idx:
            probs=self.predict(np.array([tune], dtype='int32'))
            if nobars:
                probs[baridxs] = 0 # set bar probabilities to 0
                probs = probs*(1./probs.sum()) # make the distribution sum to 1 again
            next_itoken = self.rng.choice(self.vocab_idxs, p=probs)
    ...
    def findbaridxs(self):
        # reverse idx2token to find bar idxs
        bartkns = ['|', '|:', ':|', '|1', '|2', '|:1', '|:2', ':|1', ':|2']  # this is probably close enough to exhaustive
        out = []
        for id in self.vocab_idxs:
            tkn = self.idx2token[id]
            if tkn in bartkns:
                out.append(id)
        return out
